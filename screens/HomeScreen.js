...
const HomeScreen = () => {
  ...
  useEffect(()=>{
    ...
    const fetchPosts = async()=>{
      try {
        const response = await axios.get("http://192.168.32.136:3000/get-posts")
        setPosts(response.data) 
      } catch (error) {
        console.log("Error fetching thread".error.message)
      }
    }
    console.log("shibbo posts",posts)
    ...
    const handleLike = async (postId) => {
      try {
        const response = await axios.put(
          `http://192.168.32.136:3000/posts/${postId}/${userId}/like`
        );
...
const handleDislike = async (postId) => {
  try {
    const response = await axios.put(
      `http://192.168.32.136:3000/posts/${postId}/${userId}/unlike`
    );
    const updatedPost = response.data;
